...
class PlayState extends FlxState {
	...
	var explosion:FlxTypedEmitter<FlxParticle>;
	...
	private function createInstruct():Void {
		...
		instuctTimer.start(4, function(timer:FlxTimer):Void {
			...
			FlxTween.tween(instruct, {alpha: 0}, .5, {
	...
	private function createExplosion():Void {
		...
		explosion.loadParticles("assets/images/red.png", 25, 0, true);

		explosion.angle.set(0, 360);
		explosion.acceleration.set(0, 5, 0.2, 360, 200, 1.8);
		explosion.alpha.set(1, 1, 0, 0);
		add(explosion);
	}
	...
	private function onSongEnd():Void {
		...
		FlxTween.tween(endText2, {alpha: 1}, 1, {
			onComplete: function(tween:FlxTween):Void {
				isLevelDone = true;
			}
		});
	...
	private function updateTargets():Void {
		var musicTrackTime = musicTrack.time / 1000;
		targets.forEach(function(t:Target) {
			t.updateTime(musicTrackTime);
		});
